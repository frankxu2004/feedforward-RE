...
def main():
    ...
    parser.add_argument('--vocab_size', default=35888, type=int, help='The vocabulary size for the embedding matrix.')
    ...
    word2id = data_utils.load_from_dump(os.path.join(DATA_ROOT, "basic/%d.vocab" % vocab_size))
    embedding = prepare_pretrained_embedding(emb_file, word2id, dim)
    np.save(DATA_ROOT + 'basic/emb-v%d-d%d.npy' % (vocab_size, dim), embedding)
