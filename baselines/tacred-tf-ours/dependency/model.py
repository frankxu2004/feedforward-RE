...
tf.app.flags.DEFINE_integer('vocab_size', 32857, 'Vocabulary size')
tf.app.flags.DEFINE_integer('num_class', 24, 'Number of class to consider')

tf.app.flags.DEFINE_integer('sent_len', 33, 'Input sentence length. This is after the padding is performed.')
...
def _get_lstm_graph_info():
    ...
    if FLAGS.bi:
        ...
        model_name = 'RNN'
    info = 'Building %s graph with [%d layers, %d hidden_size (%d word, %d pos, %d ner, %d deprel), %d softmax hidden size]' % \
        (model_name, FLAGS.num_layers, total_size, FLAGS.hidden_size, FLAGS.pos_size, FLAGS.ner_size, FLAGS.deprel_size, FLAGS.softmax_hidden)
    if FLAGS.pool:
...
class RNNModel(object):
    """
    A RNN model that runs over dependency paths.
    """
    ...
    def build_graph(self):
        ...
        self.labels = tf.placeholder(dtype=tf.int32, shape=[None], name='input_label')
        ...
        if FLAGS.pool:
            ...
            # attention layer
            self.attention, attn_final_state = _create_attention_layer(rnn_outputs, rnn_final_state, self.seq_lens, dim, self.batch_size)
