...
tf.app.flags.DEFINE_string('data_dir', '../data_nyt/dependency', 'Directory of the data')
tf.app.flags.DEFINE_string('train_dir', './train/rnn_nyt', 'Directory to save training checkpoint files')
...
tf.app.flags.DEFINE_string('model', 'rnn', 'Must be from rnn')
...
def train():
    ...
    for i in range(100):
        dev_loader = data_utils.DataLoader(
            os.path.join(FLAGS.data_dir, 'cv', 'dev.vocab%d.id.%d' % (FLAGS.vocab_size, i)),
            FLAGS.batch_size, FLAGS.sent_len)
        test_loader = data_utils.DataLoader(
            os.path.join(FLAGS.data_dir, 'cv', 'test.vocab%d.id.%d' % (FLAGS.vocab_size, i)),
            FLAGS.batch_size, FLAGS.sent_len)
